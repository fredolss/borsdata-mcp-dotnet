...
public static class HoldingsTools
{
    [McpServerTool, Description(
        ...
        "(non-Nordic, Pro+) instrument's insId — discover one via search_instruments with " +
        "universe:'global' — but Börsdata does not track insider disclosures for most global " +
    ...
    [McpServerTool, Description(
        ...
        "(non-Nordic, Pro+) instrument's insId — discover one via search_instruments with " +
        "universe:'global' — but Börsdata does not track buyback disclosures for most global " +
        "instruments, so a global insId typically returns an empty result (HTTP 200, not an error) " +
        "rather than failing.")]
    ...
    public static async Task<string> GetShortHoldings(
        BorsdataApiClient client,
        [Description("Comma-separated instrument insIds to restrict results to, e.g. your holdings resolved via search_instruments. Optional — omit to screen all instruments.")]
