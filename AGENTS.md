...
- Focus on the files and components relevant to the requested task.
- Prefer targeted searches and file reads over exploring the entire repository.
- Reuse existing patterns and abstractions.
- Avoid unrelated refactoring, formatting changes and documentation updates.
- Prefer targeted tests during implementation.
- Run the full test suite when changes affect shared components or when necessary to verify correctness.
- Do not repeat successful test runs without a reason.
- Keep final responses concise, covering changes made, verification results and unresolved issues.
