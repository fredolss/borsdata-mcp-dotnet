...
|---|---|---|
| `search_instruments` | Searches instruments by name, ticker, or ISIN; supports market/country/sector/branch filters, Nordic/global/all universes, deterministic sorting, and cursor-paginated results. Returns `{ totalMatched, returned, instruments, nextCursor? }` | First call: `search`, `marketId`, `countryId`, `sectorId`, `branchId`, `universe` (`nordic`/`global`/`all`), `sortBy` (`ticker`/`name`), `sortDirection` (`asc`/`desc`), `pageSize` (default 50, max 200). Later pages: `cursor` only. |
...
| `get_latest_stock_prices` | Latest available daily OHLCV records, enriched with `ticker`/`name`, sorted and cursor-paginated. Returns `{ totalMatched, returned, values, nextCursor? }` with compact Börsdata fields `i/d/h/l/c/o/v`. | First call: `instrumentIds`, `global`, `sortBy`, `sortDirection`, `pageSize` (optional). Later pages: `cursor` only. |
| `get_stock_prices_by_date` | Daily OHLCV records for a specific historical date; a non-trading day returns no results. Uses the same enriched, sorted and paginated shape as `get_latest_stock_prices`. | First call: `date` (required); `instrumentIds`, `global`, `sortBy`, `sortDirection`, `pageSize` (optional). Later pages: `cursor` only. |
...
`instrumentId` is the `insId` returned by `search_instruments`, and is used
