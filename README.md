...
| `list_kpi_metadata` | Lists all KPIs known to Börsdata (kpiId, Swedish/English name, format, whether the value is a string) — use to find the `kpiId` for `get_kpi_screener`/`get_kpi_history`/`get_kpi_list_screener`/`get_kpi_list_screener_all_instruments` | — |
...
| `get_kpi_list_screener` | A calculated KPI value (e.g. P/E) for a specific set of instruments in one call — the ones you already have insIds for (holdings, a prior `list_instruments` result, search). Each result is enriched with `ticker`/`name`. Returns `{ kpiId, calcGroup, calc, totalMatched, returned, values }` | `kpiId`, `calcGroup`, `calc`, `instrumentIds` (comma-separated) (all required); `marketId`, `countryId`, `sectorId`, `branchId`, `minValue`, `maxValue`, `sortDescending`, `global`, `maxCount` (optional — the four attribute ids narrow further on top of `instrumentIds`, resolved server-side, no need to call `list_instruments` first; `global` switches to Börsdata's non-Nordic Pro+ universe instead of the default Nordic one) |
| `get_kpi_list_screener_all_instruments` | Same calculated KPI value as `get_kpi_list_screener`, but for every instrument on Börsdata in one call — use this instead only when you do *not* already have specific instrumentIds; optionally narrow by market/sector/country/branch. Each result is enriched with `ticker`/`name`. Returns `{ kpiId, calcGroup, calc, totalMatched, returned, values }` | `kpiId`, `calcGroup`, `calc` (required); `marketId`, `countryId`, `sectorId`, `branchId`, `minValue`, `maxValue`, `sortDescending`, `global`, `maxCount` (optional — `marketId`/`countryId`/`sectorId`/`branchId` filter server-side, no need to call `list_instruments` first; omitting every filter returns ~14,000 entries; `global` switches to Börsdata's non-Nordic Pro+ universe instead of the default Nordic one) |
